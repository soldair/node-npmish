...
function list(dir,cb) {
  ...
  var tars = [];
  var find = walkdir(dir),pending = 0,ended = false;
  find.on('file',function(path,stat){
    ...
    rs.on('end',function(){
      ...
      find.emit('tar',{path:path,version:matches[2],name:matches[1],sha:sha});
      if(!pending && ended && cb) cb(undefined,tars);
    });
  });
...
function createDocument(tarVersions,host,cb){
  ...
  tarVersions.forEach(function(tarinfo,k){
        if(semver.gt(tarinfo.version,latest)) {
          latest = tarinfo.version;
          latestSha = tarinfo.sha;
        }
